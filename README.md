...
本系統已改用 **OCR.Space API** 提供更精準的繁體中文辨識能力：

- **API 金鑰設定**：點擊網頁右上角的「⚙️ 設定」按鈕，輸入您的 OCR.Space API 金鑰。金鑰可至 [OCR.Space](https://ocr.space/OCRAPI) 免費申請。您的金鑰僅會安全地儲存於手機瀏覽器本地（localStorage），不會上傳至任何第三方伺服器。
